...
modules.each(){ module ->
    cmd = cmdPrefix + """mvn install:install-file -DpomFile=${module}.pom -Dfile=${findbugsHome}/lib/${module}.jar -DgroupId=com.google.findbugs -DartifactId=${module} -Dversion=${findbugsVersion} -Dpackaging=jar"""
    proc = cmd.execute()
    println proc.text
}
